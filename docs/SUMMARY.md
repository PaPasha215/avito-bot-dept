...
## Усиление качества и guardrails (2026-03-01)
1. Закрыты критичные слабые места:
   - self-learning больше не конфликтует с правилом `WRONG_DOMAIN`;
   - кабинет не поднимается с дефолтным секретом подписи cookie;
   - daily report не помечается как отправленный, если реальной отправки не было.
2. Усилен runtime-контур:
   - router жестче отсекает явный off-topic внутри real-estate объявлений;
   - price/budget detector лучше понимает короткие ценовые форматы (`800р`, `20к`).
3. Тестовый baseline подтвержден:
   - установлен `pytest`;
   - полный прогон `python3 -m pytest` успешен;
   - текущий статус: `40 passed`.
4. Подготовлен отдельный безопасный план развития:
   - [`docs/V1_1_PLAN.md`](/Users/home/Documents/Bot/docs/V1_1_PLAN.md)
   - фокус: scorecard, feedback loop, retrieval-first self-learning, затем осторожный ML.
...
## Обновление по бизнес-правилам заселения (2026-02-18)
1. Зафиксированы адресные правила:
   - посуточное размещение только для 3 хостелов:
     - Куйбышева, 30 («Сити Е»);
     - Мамина-Сибиряка, 132 («В гостях у бабуси»);
     - Ботаническая, 30 («Уютное местечко»).
   - по остальным объектам бот ведет клиента в помесячный сценарий.
2. Цены в чате:
   - бот дает ориентировочные цены (`от/примерно`);
   - точная стоимость подтверждается менеджером.
3. Добавлен анти-абьюз фильтр:
   - сообщения с оскорблениями/угрозами/обвинениями переводятся в `IGNORE_SILENT`.
4. Подготовлено отдельное ТЗ по интерфейсу менеджеров:
   - `docs/TELEGRAM_MENU_2LEVEL_TZ.md` (двухуровневое меню без перегруза).

## Расширение на новые площадки (2026-02-18)
1. Зафиксирован универсальный checklist готовности коннектора:
   - `docs/CONNECTOR_READINESS_CHECKLIST.md`.
2. Текущая приоритизация:
   - P1: Avito (активно в проде),
   - P2: Циан (следующий кандидат при подтвержденном API-доступе),
   - P3/P4: Юла и Яндекс.Недвижимость как feed-first каналы до подтверждения chat API.

## Старт Youla (2026-02-18)
1. Добавлен отдельный onboarding:
   - `docs/YOULA_ONBOARDING.md`.
2. Добавлен шаблон обращения в поддержку:
